...
class AcademicController extends Controller
{
    ...
    public function assignSubjectSearch(Request $request)
    {
        ...
        if ($validator->fails()) {
            if (ApiBaseMethod::checkUrl($request->fullUrl())) {
                return ApiBaseMethod::sendError('Erreur de validation.', $validator->errors());
            }
            return redirect()->back()
    ...
    public function assignSubjectStore(Request $request)
    {
        ...
        try {
                ...
                if (isset($request->subjects)) {
                    foreach ($request->subjects as $subject) {
                        if ($subject != "") {
                            ...
                            $assign_subject->created_at = YearCheck::getYear() . '-' . date('d-m h:i:s');
            ...
            if (ApiBaseMethod::checkUrl($request->fullUrl())) {
                return ApiBaseMethod::sendResponse(null, 'Enregistrement mise à jour avec succès');
            }
            Toastr::success('Operation éffectuée succès', 'Succès');
            return redirect()->back();
    ...
    public function assignSubjectFind(Request $request)
    {
        ...
        if ($validator->fails()) {
            if (ApiBaseMethod::checkUrl($request->fullUrl())) {
                return ApiBaseMethod::sendError('Erreur validation.', $validator->errors());
            }
            return redirect()->back()
                ->withErrors($validator)
                ->withInput();
        ...
        try {
            ...
            if ($assign_subjects->count() == 0) {
                if (ApiBaseMethod::checkUrl($request->fullUrl())) {
                    return ApiBaseMethod::sendError('Aucun résultat trouvé');
                }
                Toastr::error('Aucun résultat trouvé', 'Echec');
                return redirect()->back();
    ...
    public function assignRoutineStore(Request $request)
    {
        ...
        try {
            ...
            Toastr::success('Operation avec succès', 'Succès');
